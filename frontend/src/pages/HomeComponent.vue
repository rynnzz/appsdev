<template>
    ...
    <q-page-container>
      <q-header elevated class="header">
        <q-toolbar>
          ...
          <q-btn flat round dense icon="notifications" aria-label="Notifications" @click="goToNotifications" style="margin: 5px; align-items: center;"></q-btn>
      ...
      <q-page class="body">
        <div class="row" style="margin: 20px;">
          <h2>Good Day, {{ userFirstName }}!</h2>
          <q-card dark bordered class="bg-grey-10 my-card" style="height: 300px; margin-bottom: 200px;">
            <q-card-section>
              <div class="text-h3">Our Mission</div>
            </q-card-section>
            <q-separator dark style="margin: 20px;" />
            <q-card-section class="q-pt-none text-h6">
              Welcome to CPC Safety Central! We're here to make school life easy. Our platform brings together
              schedules,
              announcements, and emergency contacts in one convenient space. No complicated stuff, just a user
              friendly
              interface designed to keep students in the loop effortlessly. Built with your needs in mind, CPC
              safety
              Central is all about creating a connected and informed school community. Let's prioritize safety
              and
              simplicity together!
            </q-card-section>
  ...
  <q-dialog v-model="isEditModalVisible" persistent>
    <q-card class="custom-card" style="width: 300vw;">
      ...
      <q-card-section>
        <textarea v-model="editedEvent.content" placeholder="Event" class="compose"> </textarea>
        <input type="date" v-model="editedEvent.date" class="date" />
      </q-card-section>
...
export default {
  ...
  methods: {
    ...
    async getEvents() {
      try {
        const response = await fetch('http://localhost/appsdev/backend/posts.php?action=getEvents');
