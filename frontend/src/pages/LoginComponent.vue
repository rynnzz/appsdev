<template>
  ...
  <body>
    <h3 align="right">Safeguarded, Smart, and<br>
      <div style="margin-right: 20px;"> Effortless Safety Hub</div>
    </h3>
    <div align="right" style="margin-right: 150px;">
      <form @submit.prevent="login">
        <label for="id_num"></label><br>
        <input v-model="id_num" type="text" id="id_num" name="id_num" class="id"
          placeholder="Enter School ID, Teacher's ID or Administrator ID"><br><br>
        <label for="password"></label><br>
        <input v-model="password" type="password" id="password" class="password" name="password"
          placeholder="Enter Password"><br><br>
        <div v-if="errorMessage" class="text-danger">{{ errorMessage }}</div>
        <div class="login-button">
          <q-btn type="submit" color="blue" icon="eco" style="width: 150px; height: 40px; border-radius: 10px;"
            label="Login" />
        </div>
        <div class="register-button">
          <router-link to="/RegisterComponent">
            <q-btn type="button" color="green" style="width: 190px; height: 40px; border-radius: 10px;"><i
                class="mx-2"><svg xmlns="http://www.w3.org/2000/svg" width="25" height="25" fill="currentColor"
...
import { simulateLogin, checkIfUserIsAuthenticated } from 'src/router/auth'
...
export default {
  ...
  methods: {
    async login() {
      try {
        const response = await fetch('http://localhost/appsdev/backend/login.php', {
          method: 'POST',
          headers: {
